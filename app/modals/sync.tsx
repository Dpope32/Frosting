...
if (!(global as any)._syncFetchWrapped) {
  ...
  global.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    const shouldSkipLogging =
      ...
      url.toLowerCase().includes("geocoding") ||
      url.toLowerCase().includes("weather") ||
      url.toLowerCase().includes("stoic") ||
...
export default function SyncScreen() {
  ...
  const [initialModalMode, setInitialModalMode] = useState<
  ...
  const contentWidth = isWeb
    ? width * 0.7025
    : isIpad()
    ? Math.min(width - baseSpacing * 2, 600)
    : Math.min(width - baseSpacing * 2, 350);
  ...
  const handleLeaveWorkspace = React.useCallback(() => {
    ...
    Alert.alert("Leave Workspace", "Are you sure?", [
  ...
  const onWorkspaceUpdated = React.useCallback(
    (id: string, action: "created" | "joined") => {
  ...
  const handleJoinWorkspace = React.useCallback(
    async (code: string) => {
      ...
      try {
        addSyncLog(
          `🔌 Attempting to join workspace with code: ${code}`,
          "info"
        );
        ...
        if (!wasPremium) {
          addSyncLog(
            "🔄 First-time premium activation - initializing sync",
            "info"
          );
        ...
        addSyncLog(
          `✅ Successfully joined workspace: ${result.id.slice(0, 8)}`,
          "success"
        );
        useToastStore
          .getState()
    ...
    <>
      <ScrollView
        ...
        contentContainerStyle={{
          ...
          paddingBottom: isWeb ? 100 : isIpad() ? 50 : 25,
        }}
      >
        <YStack
          gap={baseSpacing * 2}
        ...
        >
          ...
          {shouldShowSyncTable ? (
            <XStack
              ...
              marginBottom={
                premium && !workspaceId ? -baseSpacing : baseSpacing
              }
            >
              <SyncTable
                isDark={isDark}
                ...
                onCopyCurrentSpaceId={async () => {
                  ...
                  useToastStore
                    .getState()
                    .showToast("Current space ID copied", "success");
                }}
