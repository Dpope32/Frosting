...
const ChatPage = () => {
  ...
  useEffect(() => {
    if (id) {
      getMessages(db as any, parseInt(id)).then((res: Message[]) => {
        setMessages(res);
      });
    }
  }, [id]);
    ...
    const handleNewMessage = (payload: any) => {
      setMessages((messages) => {
        ...
        if (payload.choices[0]?.finishReason) {
          ...
          addMessage(db as any, parseInt(chatIdRef.current), {
  ...
  const getCompletion = async (text: string) => {
    if (messages.length === 0) {
      addChat(db as any, text).then((res: any) => {
        const chatID = res.insertId || 0;
        setChatId(chatID.toString());
        addMessage(db as any, chatID, { content: text, role: Role.User });
      });
    }
        ...
        options={{
          headerTitle: () => (
            <HeaderDropDown
              title="ChatGPT"
              items={[
                { key: '3.5', title: 'GPT-3.5', icon: 'square.and.arrow.down' },
                { key: '4', title: 'GPT-4', icon: 'square.and.arrow.up' },
              ]}
