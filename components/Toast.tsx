...
import { ToastPosition } from '@/store/ToastStore'
...
const getContainerStyle = (position: ToastPosition) => {
	return {
		...
		...(isWeb
			? position === 'top-right'
				? { top: 16, right: 20 }
				: { top: 20, left: 0, right: 0, alignItems: 'center' as const }
			: position === 'top-right'
				? { top: height * 0.085, right: 20 }
				: { top: height * 0.085, left: 0, right: 0, alignItems: 'center' as const }),
		zIndex: 100000,
...
export const Toast = React.memo(() => {
	...
	const topCenterToasts = toasts.filter(t => t.position !== 'top-right') 

	return (
		<>
			{topCenterToasts.length > 0 && (
				<YStack {...getContainerStyle('top-center')} gap="$2">
			...
			{topRightToasts.length > 0 && (
				<YStack {...getContainerStyle('top-right')} gap="$2">
...
const ToastItem: React.FC<ToastItemProps> = React.memo(({
	...
	const colorMap: Record<ToastType, string> = React.useMemo(() => ({
		success: isDark ? 'rgba(2, 255, 65, 0.86)' : 'rgba(2, 255, 65, 0.86)',
		error: '#ef4444',
		info: '#3b82f6',
		warning: '#f59e0b',
	}), [])
	...
	const backgroundMap: Record<ToastType, string> = React.useMemo(() => ({
		success: isDark ? 'rgba(7, 27, 16, 0.95)' : 'rgba(44, 94, 57, 0.52)',
	...
	return (
		...
		>
			<BlurView
				intensity={isDark ? 90 : 20}
				tint={isDark ? 'dark' : 'light'}
				style={{
					flexDirection: 'row',
					alignItems: 'center',
					paddingHorizontal: isLarge ? 14 : 12,
					paddingVertical: isWeb ? 10 :isLarge ? 14 : 12,  
