import { isIpad } from '@/utils';
import { Platform, StyleSheet } from 'react-native';
import { isWeb } from 'tamagui';

export const getMonthStyles = (webColumnCount: number, isDark: boolean) => StyleSheet.create({
  calendar: {
    borderRadius: Platform.OS === 'web' ? 24 : isIpad() ? 14 : 12,
    elevation: isWeb? 0 : 3,
    overflow: 'hidden',
    padding: 0,
    margin: isWeb ? 16 : 8,
    backgroundColor: isDark ? '#121212' : '#FFFFFF',
    borderWidth: isDark ? 2 : 1,
    borderColor: isDark ? '#333' : '#E0E0E0',
    ...(Platform.OS === 'web' ? { 
      boxShadow: isDark ? '0 4px 12px rgba(0,0,0,0.25)' : '0 2px 8px rgba(0,0,0,0.08)',
      margin: 0,
      marginBottom: 24
    } : {})
  },
  header: {
    alignItems: 'center',
    marginBottom: 0,
    height: webColumnCount === 1 ? isWeb? 72 : 48 : (webColumnCount === 2 ? isWeb ? 60 : 40 : 40),
    justifyContent: 'center',
    backgroundColor: isDark ? '#1b1b1b' : '#f7f7f7',
    borderBottomWidth: 1,
    borderBottomColor: isDark ? '#333' : '#E8E8E8',
  },
  monthText: {
    fontSize: webColumnCount === 1 ? isWeb? 26 : isIpad() ? 22 : 20 : (webColumnCount === 2 ? isWeb? 22 : isIpad() ? 18 : 20 : 16),
    fontWeight: '600',
    color: isDark ? '#FFF' : '#222'
  },
  weekDays: {
    flexDirection: 'row',
    paddingVertical: 6,
    borderBottomWidth: 1,
    borderBottomColor: isDark ? '#333' : '#E8E8E8',
    backgroundColor: isDark ? '#1c1c1c' : '#f7f7f7',
  },
  weekDayContainer: {
    flex: 1,
    alignItems: 'center',
    paddingVertical: 4,
    backgroundColor: isDark ? '#1c1c1c' : '#f7f7f7',
  },
  weekDay: {
    fontSize: webColumnCount === 1 ? isWeb? 18 : isIpad() ? 14 : 11 : (webColumnCount === 2 ? isWeb? 16 : isIpad() ? 13  : 14 : 11),
    fontWeight: '600',
    color: isDark ? '#AAA' : '#444',
  },
  daysGrid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    marginTop: 0,
    paddingTop: 0,
    paddingBottom: 0
  },
  dayCell: {
    width: '14.28%',
    aspectRatio:
      Platform.OS === 'web'
        ? (webColumnCount === 3
            ? 0.9
            : webColumnCount === 2
              ? 0.9
              : 1)
        : (webColumnCount === 3
            ? 0.8
            : webColumnCount === 2
              ? 0.7
              : 0.9),
    padding: Platform.OS === 'web' ? 3 : 1,
    borderWidth: 0.5,
    borderColor: isDark ? '#444' : '#E5E5E5',
    overflow: 'hidden',
  },
  weekendDayCell: {
    backgroundColor: isDark ? '#0f0f0f' : '#f7f7f7',
  },
  currentDateCell: {
    borderBottomWidth: 0,
  },
  pastDateCell: {
    borderBottomWidth: 0,
    position: 'relative',
    backgroundColor: isDark ? '#000000' : '#f7f7f7',
  },
  lastRowCell: {
    borderBottomWidth: 0,
  },
  holidayCell: {
    backgroundColor: isDark ? '#1E1E1E' : '#F5F5F5',
    borderRadius: 3,
    width: '100%',
    marginVertical: webColumnCount === 1 && isWeb ? 0.5 : 0,
    borderWidth: 0.5,
    borderColor: isDark ? '#555' : '#E8E8E8',
    paddingHorizontal: webColumnCount === 3 ? 1 : webColumnCount === 2 ? 2 : 3,
    paddingVertical: webColumnCount === 3 ? 1 : webColumnCount === 2 ? 1.5 : 2,
    minHeight: webColumnCount === 3 ? 12 : webColumnCount === 2 ? 14 : 16,
    maxHeight: webColumnCount === 3 ? 14 : webColumnCount === 2 ? 16 : 18,
    justifyContent: 'center',
    overflow: 'hidden',
  },
  holidayText: {
    fontSize: webColumnCount === 3 ? 7 : webColumnCount === 1 ? (isWeb ? 11 : 6) : (webColumnCount === 2 ? (isWeb ? 9 : 5) : 7),
    textAlign: 'center',
    fontWeight: '500',
    lineHeight: webColumnCount === 3 ? 8 : webColumnCount === 2 ? 10 : undefined,
  },
  taskIconContainer: {
    position: 'absolute',
    top: 0,
    left: 0,
    right: 0,
    bottom: 0,
  },  
  taskIconText: {
    fontSize: webColumnCount === 1 ? isWeb? 15 : 14 : (webColumnCount === 2 ? isWeb? 14 : 14 : 11),
    fontWeight: '600',  
    color: isDark ? '#AAA' : '#444',
  },
  dayCellContent: {
    flex: 1,
    justifyContent: 'flex-start',
    alignItems: 'center',
    position: 'relative',
    paddingHorizontal: 1,
    marginVertical: -4,
    overflow: 'visible',
    paddingTop: webColumnCount === 1 ? (isWeb ? 24 : 15) : webColumnCount === 2 ? (isWeb ? 16 : 14) : (isWeb ? 12 : 10),
    gap: webColumnCount === 1 ? (isWeb ? 2 : 0) : 1,
  },
  pastDateOverlay: {
    opacity: isDark ? 0.7 : 0.4,
    backgroundColor: isDark ? 'rgba(0, 0, 0,1)' : 'rgba(128, 128, 128, 0.3)',
  },
  pastDateStrikethrough: {
    position: 'absolute',
    width: isWeb ? '145%' : isIpad() ? '146%' : '150%',
    height: isIpad() ? 1.9 : 1,
    backgroundColor: isDark ? '#777' : '#777',
    top: isWeb ? '50%' : isIpad() ? '60%' : '69%',
    left: webColumnCount === 1 ? isWeb ? -45 : isIpad() ? -15 : -13 : webColumnCount === 2 ? isWeb ? -26 : isIpad() ? -10 : -6 : webColumnCount === 3 ? isWeb ? -20 : isIpad() ? -14 : -6 : -6,
    transform:  isIpad() ? [{ rotate: '131deg' }] : [{ rotate: '132deg' }],
    zIndex: 1000,
  },
  dayNumber: {
    position: 'absolute',
    top: Platform.OS === 'web' ? 2 : 2,
    right: Platform.OS === 'web' ? 4 : 3,
    fontSize: webColumnCount === 1 
      ? (Platform.OS === 'web' ? 16 : isIpad() ? 14 : 12) 
      : webColumnCount === 2 
        ? (Platform.OS === 'web' ? 12 : isIpad() ? 11 : 10)
        : (Platform.OS === 'web' ? 10 : isIpad() ? 9 : 8),
    fontWeight: '600',
    color: isDark ? '#FFFFFF' : '#222222',
    zIndex: 100
  },
  today: {
    borderWidth: 1.5,
    borderRadius: 6,
    marginVertical: 0,
    marginHorizontal: 0,
    padding: 0,
    zIndex: 5,
    elevation: 1,
  },
  // Common event container and text styles
  eventIconContainer: {
    width: '100%',
    alignItems: 'center',
    borderRadius: 3,
    borderWidth: 0.5,
    borderColor: isDark ? '#555' : '#E8E8E8',
    marginVertical: webColumnCount === 1 && isWeb ? 0.5 : 0,
    paddingHorizontal: webColumnCount === 3 ? 1 : webColumnCount === 2 ? 2 : 3,
    paddingVertical: webColumnCount === 3 ? 1 : webColumnCount === 2 ? 1.5 : 2,
    backgroundColor: isDark ? '#1E1E1E' : '#F5F5F5',
    minHeight: webColumnCount === 3 ? 12 : webColumnCount === 2 ? 14 : 16,
    maxHeight: webColumnCount === 3 ? 14 : webColumnCount === 2 ? 16 : 18,
    justifyContent: 'center',
    overflow: 'hidden',
  },
  eventIconText: {
    fontSize: webColumnCount === 1 
      ? (Platform.OS === 'web' ? 11 : 6) 
      : (webColumnCount === 2 ? (Platform.OS === 'web' ? 9 : 5) : 6),
    textAlign: 'center',
    fontWeight: '500',
    width: '100%',
    lineHeight: webColumnCount === 3 ? 8 : webColumnCount === 2 ? 10 : undefined,
  },
  // Dots
  indicatorContainer: {
    position: 'absolute',
    bottom: 2,
    left: 0,
    right: 0,
    flexDirection: 'row',
    gap: 2,
    justifyContent: 'center'
  },
  eventDot: {
    width: Platform.OS === 'web' ? 8 : 6,
    height: Platform.OS === 'web' ? 8 : 6,
    borderRadius: Platform.OS === 'web' ? 4 : 3,
    marginTop: Platform.OS === 'web' ? 4 : 2
  },
  // NBA
  nbaLogoContainer: {
    position: 'absolute',
    top: 2,
    left: 2,
    width: webColumnCount === 1 ? 20 : (Platform.OS === 'web' ? 16 : 12),
    height: webColumnCount === 1 ? 20 : (Platform.OS === 'web' ? 16 : 12),
    zIndex: 1
  },
  nbaLogo: {
    width: '100%',
    height: '100%'
  },
  trailingBlankCell: {
    width: '14.28%',
    aspectRatio: 1,
    backgroundColor: isDark ? '#111111' : '#f7f7f7',
    borderRightWidth: 0.5,
    borderLeftWidth: 0.5,
    borderTopWidth: 0.5,
    borderBottomWidth: 0.5,
    borderColor: isDark ? '#444' : '#E5E5E5',
    overflow: 'hidden',
  },
  blankCell: {
    backgroundColor: isDark ? '#111111' : '#f7f7f7',
    borderRightWidth: 0.5,
    borderLeftWidth: 0.5,
    borderTopWidth: 0.5,
    borderBottomWidth: 0.5,
    borderColor: isDark ? '#444' : '#E5E5E5',
  },
  trailingBlankCellLast: {
    borderRightWidth: 1
  },
  lastRowCellLeft: {
    borderLeftWidth: 1
  },
  lastRowCellRight: {
    borderRightWidth: 1
  }
});
