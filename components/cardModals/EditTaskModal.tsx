...
interface EditTaskModalContentProps {
  taskToEdit: Task;
  closeModal: () => void;
  // Add props for state and functions from the outer component
  isDark: boolean;
...
function EditTaskModalContent({
  ...
  // Initialize form state when taskToEdit prop changes
  useEffect(() => {
    // No need to check taskToEdit here, as this component only renders when it's valid
    setEditedTask({
      name: taskToEdit.name,
        ...
        dueDate: taskToEdit.dueDate, // Keep dueDate if it exists
      ...
      if (taskToEdit.time) {
        try {
          ...
          const parsedTime = parse(taskToEdit.time, 'h:mm a', new Date());
          if (!isNaN(parsedTime.getTime())) {
            initialDate = parsedTime;
          }
        } catch (e) { console.error("Error parsing task time:", e); }
      } else if (taskToEdit.recurrenceDate) {
         try {
            // Use recurrenceDate if time is not set
            const recDate = new Date(taskToEdit.recurrenceDate);
    ...
    // No need for the 'else' block here, this component only renders when taskToEdit is valid.
    // Reset other states when the task prop changes
    setShowTimePicker(false);
    setIsSubmitting(false);
  }, [taskToEdit]); // Rerun only when taskToEdit prop changes
  ...
  }, [editedTask, updateTask, closeModal, showToast, isSubmitting, taskToEdit])
...
export function EditTaskModal(): JSX.Element | null {
  // Hooks are called unconditionally here
  const { isOpen, taskToEdit, closeModal } = useEditTaskStore();
  const colorScheme = useColorScheme(); // Call hooks needed by the inner component here
  ...
  // Render the content component if the modal should be open, passing down necessary props
  return (
