        ...
        >
          ...
          >
            <YStack flex={1}>
                ...
                <XStack ai="center" gap="$3" f={1} flexWrap="wrap">
                  ...
                  {project?.status && (
                    <XStack
                      ...
                      borderWidth={0.5}
                      borderColor={isDark ? '#444' : '#ddd'}
                      backgroundColor={isDark ? 'rgba(113, 148, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)'}
                      opacity={0.8}
                      ai="center"
                    >
                      <Text color={isDark ? '$blue10' : '#333'} fontSize="$3" fontFamily="$body">
                        {project.status}
                      </Text>
                    </XStack>
                  )}
              ...
              <YStack ml={isIpad() ? 16 : 0}>
              <XStack ai="center" gap="$2" my="$2" px={isIpad() ? "$2" : "$2"}>
                <Text color={isDark ? '#666' : '#666'} fontSize={isIpad() ? 15 : "$3"} w={isIpad() ? 100 : 80} fontFamily="$body">
                  Description:
                </Text>
                <Text color={isDark ? '#f6f6f6' : '#000'} fontSize={isIpad() ? 15 : "$3"} flex={1} fontFamily="$body">
                  {project?.description || 'No description'}
                </Text>
              </XStack>
              ...
              <XStack ai="center" gap={"$2"} mb="$2" px={isIpad() ? "$2" : "$2"}>
                <Text color={isDark ? '#666' : '#666'} fontSize={isIpad() ? 15 : "$3"} w={isIpad() ? 120 : 90} fontFamily="$body">
                 Days remaining:
                </Text>
                <Text
                  color={isDark ? '#f6f6f6' : '#000'}
                  fontSize={isIpad() ? 15 : "$3"}
                  flex={1}
                  fontFamily="$body"
                >
                  {(() => {
