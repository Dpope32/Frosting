// CI version of generate-changelog-smart.js - runs non-interactively
const fs = require('fs');
...
function getRecentCommits(count = 15) {
  try {
    // For CI, use a slightly different command to get only commits since the last version tag
    // or fallback to last 15 commits if no tag is found
    try {
      // Try to find the latest version tag
      const latestTag = execSync('git describe --tags --abbrev=0').toString().trim();
      return execSync(`git log ${latestTag}..HEAD --pretty=format:"%s" --no-merges`).toString()
        .split('\n')
        .filter(msg => msg.trim() !== '');
    } catch (e) {
      // If no tag found or other error, fall back to last N commits
      return execSync(`git log -${count} --pretty=format:"%s" --no-merges`).toString()
        .split('\n')
        .filter(msg => msg.trim() !== '');
    }
  } catch (error) {
...
function getChangedFiles() {
  try {
    // In CI, get files changed since the last version tag or fall back to the last commit
    try {
      // Try to find the latest version tag
      const latestTag = execSync('git describe --tags --abbrev=0').toString().trim();
      return execSync(`git diff --name-only ${latestTag}..HEAD`).toString()
        .split('\n')
        .filter(file => file.trim() !== '');
    } catch (e) {
      // If no tag found or other error, use last commit
...
function learnFromExistingChangelog() {
  try {
    ...
    const bulletPatterns = {
      ...
      hasEmoji: allBullets.filter(b => /[\u{1300}-\u{1F6FF}]/u.test(b)).length / (allBullets.length || 1),
    };
...
// Function to automatically generate and update changelog
function updateChangelog() {
  console.log('Generate Changelog CI: Starting changelog generation...');
  ...
  // Generate note and bullets automatically
  ...
  console.log(`Generated note: "${suggestedNote}"`);
  console.log('Generated bullets:');
  suggestedBullets.forEach(bullet => console.log(`- ${bullet}`));
  ...
  const newEntry = `  {
    ...
    notes: '${suggestedNote}',
    bullets: [
${suggestedBullets.map(bullet => `      '${bullet}'`).join(',\n')}
    ],
  },`;
...
try {
  updateChangelog();
  console.log('Generate Changelog CI: Successfully generated changelog');
} catch (err) {
